...
const store = new Store({
  ...
  schema: {
    ...
    // New: allow overriding the GitHub API base (useful for GH Enterprise)
    apiBase: { type: "string", default: "https://api.github.com" }
  }
});
...
function ghHeaders() {
  ...
  const headers = {
    ...
    // Keeping version header modern & stable
...
function getApiBase() {
  const base = (store.get("apiBase") || "https://api.github.com").trim();
  return base.replace(/\/+$/, "");
}
...
async function ghJson(pathOrUrl, opts = {}) {
  const res = await ghFetch(pathOrUrl, opts);
...
async function listDirectories(owner, repo, branch, rootDir) {
  ...
  for (const item of tree.tree || []) {
    if (item.type === "tree") {
      ...
      if (!rootDir || p === rootDir || p.startsWith(rootDir + "/"))
        allDirs.add(p);
    }
  }
...
async function createDirectory(owner, repo, branch, fullPath) {
  const normalized = fullPath
    ...
    .replace(/^\/+/, "")
    .replace(/\/+$/, "");
  const url = `/repos/${owner}/${repo}/contents/${encodeURIComponent(
    normalized + "/.gitkeep"
  )}`;
  const body = {
...
async function uploadFile({
  ...
  while (attempt < 50) {
    ...
    attempt += 1;
    pathInRepo =
      (safeDir ? `${safeDir}/` : "") + `${base}-${attempt}${ext}`;
  }
  ...
  const raw = `https://raw.githubusercontent.com/${owner}/${repo}/${encodeURIComponent(
    branch
  )}/${pathInRepo}`;
  const blob = `https://github.com/${owner}/${repo}/blob/${encodeURIComponent(
    branch
  )}/${pathInRepo}`;
  return {
    path: pathInRepo,
...
async function deleteFile(owner, repo, branch, pathInRepo, message) {
  const metaUrl = `/repos/${owner}/${repo}/contents/${encodeURIComponent(
    pathInRepo
  )}?ref=${encodeURIComponent(branch)}`;
  ...
  return ghJson(url, { method: "DELETE", body: JSON.stringify(body) });
}
...
async function renameFile(owner, repo, branch, oldPath, newPath) {
  ...
  if (!rawUrl)
    rawUrl = `https://raw.githubusercontent.com/${owner}/${repo}/${encodeURIComponent(
      branch
    )}/${oldPath}`;
  ...
  // put new
  const putUrl = `/repos/${owner}/${repo}/contents/${encodeURIComponent(
    newPath
  )}`;
  await ghJson(putUrl, {
    method: "PUT",
    body: JSON.stringify({
      message: `chore: rename ${path.basename(oldPath)} → ${path.basename(
        newPath
      )}`,
      content: base64Content,
      branch,
  ...
  await deleteFile(
    ...
    branch,
    oldPath,
    `chore: remove ${path.basename(oldPath)} after rename`
  );
...
function createWindow() {
  ...
  const sendMaxState = () =>
    win.webContents.send("win:max-state", win.isMaximized());
...
function createUploadWindow() {
  ...
  up.webContents.setWindowOpenHandler(({ url }) => {
    shell.openExternal(url);
    return { action: "deny" };
  });
  return up;
...
ipcMain.handle("settings:get", () => {
  return {
    ...
    apiBase: store.get("apiBase") // expose for About/Advanced UIs if needed
  };
});
...
ipcMain.handle("settings:save", (_e, payload) => {
  const allowed = [
    ...
    "committerName",
    "committerEmail",
    "apiBase" // NEW: persist override when provided
  ];
  for (const k of allowed) {
    if (payload[k] !== undefined)
      store.set(k, String(payload[k] || "").trim());
  }
  if (payload.token !== undefined)
    store.set("token", String(payload.token || "").trim());
  return true;
});
...
ipcMain.handle("settings:test", async () => {
  ...
  await ghJson(`/repos/${owner}/${repo}/branches/${encodeURIComponent(branch)}`);
  return {
    ok: true,
...
ipcMain.handle("github:listDirs", async () => {
  ...
  if (!owner || !repo || !branch)
    throw new Error("Configure owner/repo/branch in Settings.");
...
ipcMain.handle("github:rename", async (_e, { oldPath, newPath }) => {
  const owner = store.get("owner"),
    repo = store.get("repo"),
    branch = store.get("branch");
  if (!owner || !repo || !branch)
...
ipcMain.handle("upload:notify", () => {
  BrowserWindow.getAllWindows().forEach((w) =>
    w.webContents.send("upload:completed")
  );
  return true;
});
